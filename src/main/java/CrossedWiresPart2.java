...
public class CrossedWiresPart2 {
    ...
    private void executeCommands(){
        ...
        for (String[] wiresCommand : wiresCommands) {
                ...
                switch (commandCode) {
                    case "R":
                        for (int i = 0; i < moveAmount; i++) {
                            ++x;
                            wireCoordinates.add(new Point(x, y));
                        }
                        break;
            ...
            wiresCoordinates.add(wireCoordinates);
        }
    }

    private void getIntersections() {
        System.out.println("Calculating all intersections... 2%");
        intersectionList = new ArrayList<Point>(wiresCoordinates.get(0));
        intersectionList.retainAll(wiresCoordinates.get(1));
        System.out.println("Intersections calculated... 80%");
    }

    private void getShortestIntersection(){
        System.out.println("Finding shortest intersection... 81%");
        int smallestStepSum = Integer.MAX_VALUE;
        int steps = 0;

        for (Point step: wiresCoordinates.get(0)){
            ++steps;
                if (intersectionList.contains(step)){
                    Optional<Integer> otherWireSteps = getOtherWireSteps(step);
                    if (otherWireSteps.isPresent()){
//                        System.out.println(steps);
//                        System.out.println("Wire 1 step:"+ step.toString());
                        int totalSteps = steps + otherWireSteps.get();
                        if(totalSteps < smallestStepSum) smallestStepSum = totalSteps;
                    }

                }
        }
        System.out.println("Smallest step amount is: " + smallestStepSum);
    }

    private Optional<Integer> getOtherWireSteps(Point foundIntersection){
        int steps = 0;
        for (Point step: wiresCoordinates.get(1)){
            ++steps;
            if (step.equals(foundIntersection)){
//                System.out.println("Wire 2 step:"+ step.toString());
    ...
    public static void main(String[] args) {
        CrossedWiresPart2 cross = new CrossedWiresPart2();
        cross.makeCommandArrays();
        cross.executeCommands();
        cross.getIntersections();
        cross.getShortestIntersection();
    }
}
