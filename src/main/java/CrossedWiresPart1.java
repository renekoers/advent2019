...
public class CrossedWiresPart1 {
    ...
    private void getIntersections() {
        ArrayList<Point> intersectionList = wiresCoordinates.get(0);
        intersectionList.retainAll(wiresCoordinates.get(1));
        System.out.println(intersectionList.toString());

        int closest = intersectionList.stream().map(point -> Math.abs(point.x) + Math.abs(point.y)).min(Integer::compare).get();

        System.out.println(closest);
    }

    public static void main(String[] args) {
        CrossedWiresPart1 cross = new CrossedWiresPart1();
